...
{
    public class ViewModelUpdateTutor : INotifyPropertyChanged
    {
        ...
        async void updateUser()
        {
            ...
            List<Tutor> lister = await db.GetAllTutors();
            for (int n = 0; n < lister.Count; n++)
            {
                if (lister[n].tutorID.Equals(AuthorizationPage.typeUserId))
                {
                    ...
                    await db.UpdateTutor(lister[n]);
                }
            }
            await Application.Current.MainPage.Navigation.PushAsync(new UserUpdatePageTutor());
        }
